...
from theme_distinguish.corpus import get_corpus
from theme_distinguish.config import get_config
from theme_distinguish.util import to_json
...
class Interrogative:
    ...
    def __init__(self,theme):
        self.corpus = get_corpus()  # 从xls文件中读取数据，需要传入theme
    ...
    def initialize_model(self):
        ...
        """
        train, label = self.corpus.generator(theme=self.theme)
    ...
    def train(self):
        ...
        self.save_model(name=self.theme)

    def predict(self, sentence):
        """
        predict the prob of sentence if it is interrogative.
        """
        assert isinstance(sentence, str), 'Sentence must be unicode.'
        if not self.model:
            self.load_model(name=self.theme)
    ...
    def transform(self, sentence):
        ...
        if not self.vectorizer:
            self.load_model('{}_tfidf_vectorizer'.format(self.theme), 'vectorizer')
